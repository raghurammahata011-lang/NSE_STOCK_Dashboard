...
def run_streamlit_app():
    ...
    fetch_clicked = st.button("🚀 Fetch & Analyze Data", type="primary", width='stretch')
    ...
    if fetch_clicked:
        ...
        if data:
            ...
            if not df.empty:
                ...
                with tab1:
                    if show_oi_chart:
                        st.plotly_chart(create_oi_chart(df), width="stretch")
                
                with tab2:
                    if show_sentiment_chart:
                        st.plotly_chart(create_sentiment_chart(df), width="stretch")
                
                with tab3:
                    if show_iv_chart:
                        st.plotly_chart(create_iv_comparison_chart(df), width="stretch")
                
                with tab4:
                    if ml_enabled and ml_results:
                        col1, col2 = st.columns(2)
                        with col1:
                            st.plotly_chart(create_ml_prediction_chart(df, analytics, top_calls, top_puts), width="stretch")
                        with col2:
                ...
                st.dataframe(styled_option_chain(df), width="stretch", height=400)
                ...
                with col_calls:
                    ...
                    st.dataframe(top_5_calls, width="stretch", height=300)
                with col_puts:
                    st.write("📉 Top 5 Puts")
                    st.dataframe(top_5_puts, width="stretch", height=300)
